...
It mostly features ICP-type techniques (ICP-MS, ICP-OES, etc.) capable of parts-per-billion ($$\mathrm{ppb}$$) detection limits.
While LIBS cannot offer $$\mathrm{ppb}$$-level LoDs, it has the key advantage of requiring no extensive sample preparation.
...
Our goal was to adapt and expand their methodology to a higher-dimensional space, accounting for $$10$$ experimental parameters while aiming to minimize the Relative Standard Deviation (RSD) of a Cerium emission line ($$418.66\,\mathrm{nm}$$):
...
The rationale for minimizing RSD on the Cerium line is clear: we are working with an imbalanced matrix ($$100\,\mathrm{g/L}$$ Neodymium vs. $$8.5\,\mathrm{g/L}$$ Cerium). 
...
With this Constrained BO approach, we achieved a stable $$(1.5\text{--}2)\%$$ RSD regime for the Cerium line (compared to an LHS median of $$\sim 6\%$$) while significantly improving SNR ($$\approx \times 2$$).
